...
export const MOUNTS: MountConfig[] = [
  {
    ...
    // no tool of their own), and the own services page as a read and one write
    // (get_/set_linkedin_account_my_services).
    maxTools: 29,
  ...
  },
  {
    ...
    instructions:
      'GTM LinkedIn Recruiter inbox, the third messenger next to LinkedIn and Sales Navigator: read a team account\'s own Recruiter seat and hiring projects, sync its recruiter inbox into the shared conversations / messages tables (messenger_type recruiter, candidates known by talent_id), read the latest threads and one thread\'s messages, and send Recruiter InMails (a new thread, or a reply into one). Needs a Recruiter seat on the account (search accounts with recruiter_status: active) and its stamped seat number; the tools answer 422 recruiter_required / recruiter_seat_unresolvable otherwise (a member on several contracts binds one with get/select recruiter contracts). LinkedIn Recruiter asks for the account\'s password again about every 30 days: the account\'s recruiter_status turns signed_out, the tools answer 409 recruiter_reauth_required whose context.account_url is the account\'s page where the seat holder presses Sign in to Recruiter (the cloud browser opens on Recruiter), and the status is active again once they have; nobody else can sign in for them and the platform never holds the password. Stored history is searchable on the messaging mount with filter.messenger_type = recruiter. Sends are protected (preview then confirm) and spend the contract\'s InMail credits: the account\'s recruiter_inmail_credits, refreshed by check_linkedin_account_premium_subscription (its inmail_credits step) and zero means the next new thread is refused before the wire.',
    selectors: [
  ...
  },
  {
    ...
    //
    // 2026-09-05: the LinkedIn Recruiter people search took two of them, 23/25:
    // the search itself and its facet typeahead, a third id space next to the
    // flagship and Sales Navigator lookups. It lives HERE like every other live
