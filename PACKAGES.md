...
| browsers | `/mcp/linkedin/browsers` | antidetect_browsers (13), antidetect_browser_proxies (3), antidetect_browser_logs (1), cloud_browsers (1), cloud_browser_sessions (2) | 20 | ✅ |
...
2026-09-01 - the linkedin counts here went 159 to 168, which is this file catching up rather than
the surface changing. The table was last regenerated on 2026-08-12 and three packages grew under it:
`linkedin_posting` 3 to 8 (the undo half of every write verb on 2026-08-20, then the scheduled-queue
pair on 2026-08-21: `delete_linkedin_post`, `delete_linkedin_comment`, `unreact_linkedin_post`,
`get_linkedin_scheduled_posts`, `delete_linkedin_scheduled_post` - the group is still stateless, the
queue lives on LinkedIn and nothing is stored on this service), `antidetect_browsers` 10 to 13
(`list_antidetect_browser_proxy_countries` and `check_antidetect_browser_proxy` on 2026-08-20,
`get_antidetect_browser_seat_usage` on 2026-08-28) and `linkedin_messages` 12 to 13
(`delete_linkedin_message_on_linkedin`, the recall verb, on 2026-08-20). Every budget absorbed it
...
2026-09-01 - the id counts were stale in both directions and are now read off the registry: billing
