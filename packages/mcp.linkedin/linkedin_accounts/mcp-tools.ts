...
export const linkedinAccountsTools: ToolDefinition[] = [
  ...
  {
    ...base,
    name: 'get_linkedin_account_my_services',
    description:
      "Read the connected account's OWN LinkedIn services page (\"Providing services\") as its edit form: status (ACTIVE or UNPUBLISHED), description, the offered services with their standardized-skill ids, the location and messaging flags, review visibility, pricing, the currencies LinkedIn offers and the 16 top-level service categories (valid service ids, though sub-skills exist too: lookup_linkedin_param_id with type service_category finds them by name). result.page is null when the account has no page. The public page of any member is enrich_linkedin_person_services; this is the owner's view, read live.",
    toolClass: 'trivial',
    route: { service: 'linkedin', method: 'POST', pathTemplate: '/api/linkedin-accounts/{sid}/get-my-services', sidParam: 'sid' },
    operation: 'action',
    envelope: 'action',
    availability: 'ga',
    dangerous: false,
    ...
    outputSchema: McpActionResponse(LinkedinAccount, z.object({
      status: z.string().describe('Plugin dispatch status for this read.'),
      page: z.object({
        page_id: z.string().describe("The page's NUMERIC id (the form's), not the vanity slug an unpublish takes."),
        status: z.string().describe('ACTIVE (published) or UNPUBLISHED as LinkedIn answers it.'),
        description: z.string(),
        services_offered: z.array(z.object({ id: z.string(), name: z.string() })),
        location_profile: z.boolean(),
        location_remote: z.boolean(),
        location_label: z.string().nullable(),
        open_messaging: z.boolean(),
        review_visibility: z.boolean().nullable().describe('null while the page is unpublished: the form drops the element.'),
        pricing: z.object({ kind: z.string(), currency: z.string().nullable(), amount: z.number().nullable() }).passthrough(),
        available_currencies: z.array(z.string()),
        top_level_services: z.array(z.object({ id: z.string(), name: z.string() })),
      }).passthrough().nullable().describe('The form, or null when the account has no services page.'),
    }).passthrough()),
    annotations: { title: 'Get my services page', ...RO },
  },
  {
    ...base,
    name: 'set_linkedin_account_my_services',
    description:
      "Create, update, republish or unpublish the connected account's OWN LinkedIn services page (\"Providing services\"): one tool, the page's state picks the LinkedIn action. No page yet: description and services (1..10 standardized-skill ids; get_linkedin_account_my_services lists the 16 top-level ones) are required and the page is created live with LinkedIn's onboarding defaults for what you omit. A page exists: send only what changes, the rest keeps its stored value; a page that was unpublished comes back on the profile with any save, and republish: true with no field does just that (LinkedIn has no publish action). unpublish: true takes the page down (page_slug optional; the platform resolves it while the page is still published). pricing is {kind: contact} or {kind: starting_at, currency, amount}; at least one of location_profile / location_remote must stay true. Spends one edit_profile slot.",
    toolClass: 'typical',
    route: { service: 'linkedin', method: 'POST', pathTemplate: '/api/linkedin-accounts/{sid}/my-services', sidParam: 'sid' },
    operation: 'action',
    envelope: 'action',
    availability: 'ga',
    dangerous: true,
    ...
    inputSchema: z.object({
      sid: SID,
      unpublish: z.boolean().optional().describe('Take the page down. Nothing else travels with it.'),
      republish: z.boolean().optional().describe('Put an unpublished page back on the profile without changing it.'),
      page_slug: z.string().regex(/^[A-Za-z0-9_-]{1,100}$/).optional().describe('With unpublish: the page slug (the tail of /services/page/<slug>) when you have it; needed once the page is already unpublished.'),
      description: z.string().max(500).optional().describe('Required for a new page. LinkedIn\'s cap is 500.'),
      services: z.array(z.union([z.string().regex(/^\d+$/), z.object({ id: z.string().regex(/^\d+$/), name: z.string().min(1).optional() })])).min(1).max(10).optional()
        .describe('1..10 standardized-skill ids (e.g. 220 Consulting, 71 Accounting), no id twice. Required for a new page. On an existing page an id may come with its name.'),
      location_profile: z.boolean().optional().describe('Offer the services at the profile\'s own location.'),
      location_remote: z.boolean().optional().describe('Offer them remotely.'),
      open_messaging: z.boolean().optional().describe('Let members outside the network message the account for free.'),
      review_visibility: z.boolean().optional().describe('Show ratings and reviews to all members.'),
      pricing: z.object({
        kind: z.enum(['contact', 'starting_at']),
        currency: z.string().regex(/^[A-Z]{3}$/).optional().describe('ISO 4217, one of the read\'s available_currencies; with starting_at.'),
        amount: z.number().int().min(1).optional().describe('The hourly rate in whole units; with starting_at.'),
      }).optional(),
      ...usageMetaField,
    }),
    outputSchema: McpActionResponse(LinkedinAccount, z.object({
      activity_log: z.object({}).passthrough().describe('The write\'s dispatch row: create_my_services, update_my_services or unpublish_my_services (the form read before it has its own row).'),
      action: z.string().describe('created, updated, republished or unpublished.'),
      page: z.object({}).passthrough().nullable().describe('The page\'s form as read BEFORE the write; null when there was no page.'),
      page_url: z.string().nullable().describe('What the save answered: the page on a plain update, the marketplace inbox on a republish; null on a create and an unpublish.'),
      page_id: z.string().nullable().describe('The page\'s vanity slug out of page_url, what an unpublish takes; null when the answer carried none.'),
      share_text: z.string().nullable().describe('The share-box text LinkedIn pre-fills after a save, when it offers one.'),
      ok: z.boolean(),
      message: z.string().nullable().describe('The toast of the onboarding create, when LinkedIn showed one.'),
      published: z.boolean().describe('The page was unpublished and this call put it back.'),
    }).passthrough()),
    annotations: { title: 'Set my services page', ...DANGER },
  },
  {
    ...base,
