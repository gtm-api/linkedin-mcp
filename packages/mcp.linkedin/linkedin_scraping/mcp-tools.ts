...
export const linkedinScrapingTools: ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...requestBase,
      post: z.string().max(512).describe('The post, named any way you have it. An `urn:li:activity:<id>`, a feed permalink (`/feed/update/urn:li:activity:<id>/`), the share link the copy-link button produces (`/posts/<slug>-activity-<id>-<hash>`), a `urn:li:ugcPost:<id>` (company-page posts, newsletter issues) or a `urn:li:groupPost:<group>-<post>` is parsed locally and read in one wire page. A `urn:li:share:<id>` costs one extra post read first (cached 7 days, so only the first call on a post pays it): LinkedIn files a share post\'s comments under its activity urn and answers an empty list by the share urn; result.post.post_ln_id is the urn the list was read by. A linkedin.com link with no id in it (a bare slug URL) adds one more page load to find the urn. An `lnkd.in` shortlink (the form Copy link to post mints) is expanded server-side and its slug id lifted locally, one wire page like any id-carrying URL; one that expands to a non-LinkedIn target is refused 422 post_not_resolvable, and so is a share urn LinkedIn returns no post for (deleted, or not visible to the executing account). The post does NOT need to be tracked or owned by you.'),
