...
import { callableSchema, registeredShape, validationFailedResult } from './input-schema';
...
export function registerFacadeTools(
  ...
  server.registerTool(
    ...
    async (args): Promise<ToolResult> => {
      ...
      const parsed = callableSchema(entry.tool).safeParse(args.arguments ?? {});
      if (!parsed.success) return validationFailedResult(entry.tool, parsed.error);

      const ctx: DispatchContext = { tool: entry.tool, args: parsed.data, scope: getAuthScope(), deps };
      return run(ctx);
    },
  );
}
